...
class FirstBlock(nn.Module):
  def __init__(
      self, target, n_inputs, n_outputs, kernel_size, stride, dilation, padding
  ):
...
class TemporalBlock(nn.Module):
  def __init__(
      self, n_inputs, n_outputs, kernel_size, stride, dilation, padding
  ):
...
class LastBlock(nn.Module):
  ...
  def forward(self, x):
    out = self.net(x)
    return self.linear(out.transpose(1, 2) + x.transpose(1, 2)).transpose(
        1, 2
    )  #residual connection


class DepthwiseNet(nn.Module):
  """Depthwise separable temporal convolutional network with dilated convolutions.
  
  Args:
      target: Target variable index
      num_inputs: Number of input variables
      num_levels: Number of network levels
      kernel_size: Size of the convolving kernel
      dilation_c: Dilation coefficient for temporal convolutions
  """
  def __init__(
      self, target, num_inputs, num_levels, kernel_size=2, dilation_c=2
  ):
